...
glCanvas.addEventListener("mousemove", evt => {
	...
	let lastEvent = eventQueue[eventQueue.length - 1];
...
function Java_org_lwjgl_opengl_GL11_nglGetString(lib, id, funcPtr)
{
	...
	{
		// TODO: Do we need any?
		return "GL_ARB_multitexture GL_ARB_texture_env_combine";
	}
	else if(id == 0x1F02)
	{
		return "2.1 WebGL";
...
function Java_org_lwjgl_opengl_GL11_nglGetIntegerv(lib, id, memPtr, funcPtr)
{
	...
	var buf = new Int32Array(v.buffer, Number(memPtr), 16); // Allow up to 16 ints
	...
	else if(id == /*GL_MAX_TEXTURE_UNITS*/0x84E2 || id == /*GL_MAX_TEXTURE_IMAGE_UNITS*/34930 || id == /*GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS*/35660 || id == /*GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS*/35661)
	{
		buf[0] = 4;
	}
	else if(id == 34921) /* GL_MAX_TEXTURE_COORDS */
	{
		buf[0] = 4;
	}
	else if(verboseLog)
...
{
	checkNoList(curList);
	if (pname == 0x8191 /*GL_GENERATE_MIPMAP*/ || pname == 0x813D /*GL_TEXTURE_MAX_LEVEL*/) {
		return;
	}
...
function Java_org_lwjgl_opengl_GL11_nglTexImage2D(lib, target, level, internalFormat, width, height, border, format, type, memPtr, funcPtr)
{
	...
	// Convert internal formats if necessary
	if (internalFormat == 4) { // GL_RGBA4 / RGBA
		internalFormat = glCtx.RGBA;
	} else if (internalFormat == 3) {
		internalFormat = glCtx.RGB;
	}

	let needsSwizzle = false;
	if (format == 0x80E1 /*GL_BGRA*/) {
		format = glCtx.RGBA;
		internalFormat = glCtx.RGBA;
		needsSwizzle = true;
	}
	if (internalFormat == 0x80E1) {
		internalFormat = glCtx.RGBA;
	}

	// 0x8367 = GL_UNSIGNED_INT_8_8_8_8_REV
	// 0x8035 = GL_UNSIGNED_INT_8_8_8_8
	if (type == 0x8367 || type == 0x8035) {
	...
	var buf = null;
	if (Number(memPtr) !== 0) {
		// Calculate size based on format and type. For simplicity, assume 4 bytes per pixel for RGBA/RGB+padding
		let bytesPerPixel = (format === glCtx.RGB) ? 3 : 4;
		let size = width * height * bytesPerPixel;

		if (needsSwizzle) {
			// Copy buffer to avoid mutating JVM heap memory
			buf = new Uint8Array(v.buffer, Number(memPtr), size).slice();
			...
			buf = new Uint8Array(v.buffer, Number(memPtr));
		}
	}
...
{
	if(verboseLog)
		console.log("glTexCoordPointer", size, type, stride, memPtr);
...
function Java_org_lwjgl_opengl_GL11_nglNormalPointer(lib, type, stride, memPtr, funcPtr)
{
	...
	normalData.pointer = Number(memPtr) >>> 0;
}
...
function Java_org_lwjgl_opengl_GL13_glClientActiveTexture(lib, texture)
{
	checkNoList(curList);
	if(verboseLog)
		console.log("glClientActiveTexture");
...
function Java_org_lwjgl_opengl_GL13_nglClientActiveTextureARB(lib, texture, funcPtr)
{
	checkNoList(curList);
	if(verboseLog)
		console.log("glClientActiveTextureARB");
}

function Java_org_lwjgl_opengl_GL13_glClientActiveTextureARB(lib, texture)
{
	checkNoList(curList);
	if(verboseLog)
		console.log("glClientActiveTextureARB");
...
function Java_org_lwjgl_opengl_LinuxDisplay_nSetClassHint(env, clazz, display, window, res_name, res_class) {
    // Stub implementation to prevent UnsatisfiedLinkError
}

function Java_org_lwjgl_opengl_GL11_nglGetTexLevelParameteriv(lib, target, level, pname, params, funcPtr) {
    checkNoList(curList);
    var v = lib.getJNIDataView();
    var offset = Number(params);

    if (pname == 0x1000) { // GL_TEXTURE_WIDTH
        v.setInt32(offset, 256, true);
    } else if (pname == 0x1001) { // GL_TEXTURE_HEIGHT
        v.setInt32(offset, 256, true);
    } else if (pname == 0x1003) { // GL_TEXTURE_INTERNAL_FORMAT
        v.setInt32(offset, 0x1908, true); // GL_RGBA
    } else {
        v.setInt32(offset, 0, true);
    }
}
